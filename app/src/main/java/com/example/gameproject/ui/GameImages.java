...
public enum GameImages implements BitmapMethods {
    ...
    GameImages(int resID, int x, int y, int width, int height, float Xmultiplier, float Ymultiplier) {
        options.inScaled = false;
        atlas = BitmapFactory.decodeResource(MainActivity.getGameContext().getResources(), resID, options);
        image = getMultiplierBitmap(Bitmap.createBitmap(atlas, x, y, width, height), Ymultiplier, Xmultiplier);
    }
